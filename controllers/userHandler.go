...
// GetAllUser : service qui retourne la liste complÃ¨te des utilisateurs
...
func SearchUserByID(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	stringID := vars["id"]
	// FIXME : comment je passe d'une string Ã  un int64 ?
	ID, e := strconv.Atoi(stringID)
	if e != nil {
		errorResponse(&HTTPerror{Code: http.StatusBadRequest, Message: "ParamÃ¨tre id obligatoire non vide"}, http.StatusBadRequest, w)
	} else {
...
// CreateUser : RÃ©ponse sur requete POST a /user avec l'utilisateur en JSON dans le body
...
// DeleteUserID : reponse http Ã  la demande de suppression d'un utilisateur a partir de son ID
...
//UserAuthenticate : authentification de l'utilisateur, un utilisateur est en paylaod de la requÃªte
	...
	if err != nil {
		...
		if err := r.Body.Close(); err != nil {
			...
			if err := json.Unmarshal(body, &user); err != nil {
				...
				log.Printf("Utilisateur trouvÃ© :  %v\n\n", user)
...
// UserLogout : traitement de la requÃªte http DELTE sur /user/logout. Il s'agit de la dÃ©connexion de l'utilisateur
func UserLogout(w http.ResponseWriter, r *http.Request) {
	// en cas de sauvegarde de l'utilisateur connectÃ© en base il faut le supprimer
	// suppression du cookie jwt
	http.SetCookie(w, &http.Cookie{
		Name:  "jwt",
		Value: "token",
		Path:  "/",
		// FIXME : suppression de temps Ã  un time ?
...
// cette fonction ne fonctionne pas, comment tester correctement qu'une chaine de caractÃ¨re est vide ????
